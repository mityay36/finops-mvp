...
const SEVERITIES: RecSeverity[] = ['critical', 'high', 'medium', 'low']
const STATUSES: RecStatus[] = ['open', 'applied', 'dismissed', 'stale']
...
export default function Recommendations() {
  ...
  const aggregateSavings = useMemo(() => items
    ...
    .reduce((sum, i) => sum + (parseFloat(i.monthly_impact) || 0), 0), [items])

  const openCount = useMemo(() => items.filter(i => i.status === 'open').length, [items])

  const sorted = useMemo(() => {
    const sevOrder: Record<RecSeverity, number> = { critical: 0, high: 1, medium: 2, low: 3 }
    return [...items].sort((a, b) => {
      const s = sevOrder[a.severity] - sevOrder[b.severity]
      if (s !== 0) return s
      return (parseFloat(b.monthly_impact) || 0) - (parseFloat(a.monthly_impact) || 0)
    })
  }, [items])
  ...
  const triggerRefresh = async () => {
    ...
    try {
      await api.refreshRecommendations(currentClusterId)
      setTimeout(() => {
        setRefreshKey(k => k + 1)
        setRefreshing(false)
      }, 1500)
    ...
    <div className="p-6 flex flex-col gap-6 max-w-[1400px]">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--color-muted)] mt-0.5">
            Открытых: {openCount} · Потенциал экономии: <span className="font-medium text-[var(--color-accent-savings)]">{fmtMoney(aggregateSavings, currency)} / мес</span>
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      <Card padding={false}>
        {list.loading ? (
          ...
          <div className="p-10 text-center text-sm text-[var(--color-accent-critical)]">
            Ошибка: {list.error}
          </div>
        ) : sorted.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-xs text-[var(--color-muted)]">
              Попробуйте изменить фильтры или нажмите «Пересчитать».
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-[var(--color-muted)] uppercase tracking-wide">
                  ...
                  <th className="text-left font-medium px-2 py-2.5">Updated</th>
                ...
                {sorted.map(item => (
                  ...
                  >
                    ...
                    <td className="px-2 py-3 max-w-md">
                      ...
                      <div className="text-xs text-[var(--color-muted)]">{item.rule_id}</div>
                    </td>
                    <td className="px-2 py-3 text-xs">
                      <span className="text-[var(--color-muted)]">{item.target_kind}</span>
                      {item.target_namespace && <> · {item.target_namespace}</>}
                      {item.target_name && <> / <span className="font-medium">{item.target_name}</span></>}
                    </td>
                    <td className="px-2 py-3 text-right tabular font-medium"
                        style={{
                          color: item.impact_kind === 'saving'
                            ? 'var(--color-accent-savings)'
                            : 'var(--color-accent-warning)',
                        }}>
                      {item.impact_kind === 'saving' ? '−' : '+'}{fmtMoney(Math.abs(parseFloat(item.monthly_impact) || 0), item.currency)}
                    </td>
                    <td className="px-2 py-3 text-xs text-[var(--color-muted)]">
